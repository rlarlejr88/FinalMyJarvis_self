...
public class JwtUtils {
	...
	public Object validateToken(String token) {
		...
		try {
			...
			// 2. 토큰 해석
			Claims claims = Jwts.parserBuilder().setSigningKey(key) // 해석에 필요한 Key
	...
	public String getMemberNo(String token) {
		Claims claims = Jwts.parserBuilder().setSigningKey(Keys.hmacShaKeyFor(jwtSecretKey.getBytes())).build()
				.parseClaimsJws(token).getBody();
		Object memberNo = claims.get("memberNo");
		return memberNo != null ? memberNo.toString() : null;
	}
